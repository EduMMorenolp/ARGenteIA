...
export function createClient(
  ...
  if (!modelCfg && provider === "ollama") {
    ...
    const otherOllamaKey = Object.keys(config.models).find((k) =>
      k.startsWith("ollama/"),
    );
    const baseUrl =
      config.models[otherOllamaKey || ""]?.baseUrl ||
  ...
  if (!modelCfg)
    throw new Error(`Modelo "${modelKey}" no encontrado en config.models`);
...
export async function getOllamaModels(config: Config): Promise<string[]> {
  ...
  const ollamaKey = Object.keys(config.models).find((k) =>
    k.startsWith("ollama/"),
  );
  const baseUrl =
    config.models[ollamaKey || ""]?.baseUrl || "http://localhost:11434/v1";
