...
const GatewayConfigSchema = z.object({
  port: z.number().int().min(1024).max(65535).default(19666),
});
...
export function loadConfig(configPath?: string): Config {
  ...
  for (const [name, cfg] of Object.entries(models) as [string, ModelConfig][]) {
    if (cfg.apiKey && (cfg.apiKey.includes('...') || cfg.apiKey.startsWith('tu_'))) {
      console.warn(
        chalk.magenta(`⚠️  Advertencia: La API Key para "${name}" parece ser un placeholder.`),
      );
    }
  }
