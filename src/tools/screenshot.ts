...
export function registerScreenshotTool(config: Config): void {
  registerTool({
    isEnabled: () => true,
    spec: {
      type: "function",
      function: {
        ...
        description: "Captura una imagen (screenshot) de la pantalla actual del escritorio de la PC y la envía por Telegram.",
        parameters: {
          type: "object",
          properties: {
            caption: {
    ...
    handler: async (args, context) => {
      ...
      try {
        ...
        return `Error al capturar o enviar el screenshot: ${err.message}`;
      }
    },
  });
}
