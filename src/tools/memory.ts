...
export function registerMemoryTools(): void {
  ...
  registerTool({
    ...
    spec: {
      ...
      function: {
        name: 'memorize_fact',
        description:
          'Guarda un dato importante sobre el usuario para recordarlo en el futuro (a largo plazo).',
        parameters: {
          type: 'object',
          properties: {
            fact: {
              type: 'string',
              description:
                "El dato o hecho a recordar. Ej: 'A Eduardo le gusta el café sin azúcar'.",
            },
          },
          required: ['fact'],
        },
  ...
  registerTool({
        ...
        parameters: {
          ...
          properties: {
            id: {
              type: 'number',
              description:
                'El ID numérico exacto del dato a eliminar. Debes obtenerlo primero llamando a recall_facts.',
            },
          },
          required: ['id'],
        },
    ...
    handler: async (args, context) => {
      ...
      return success
        ? `He olvidado el dato con ID ${id}.`
        : `No encontré ningún dato con ID ${id} para este usuario.`;
    },
  });
}
