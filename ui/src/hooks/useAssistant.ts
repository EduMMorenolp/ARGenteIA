...
export function useAssistant() {
  ...
  const addMessage = useCallback((role: 'user' | 'assistant', text: string, model?: string, type: 'message' | 'command' | 'error' = 'message', origin?: 'web' | 'telegram') => {
  ...
  const renderContent = (text: string) => ({ __html: marked.parse(text) as string });
  ...
  return {
    ...
    handleInputChange, renderContent, logout
  };
}
